...
with st.sidebar:
    ...
    st.subheader("Potential Usage")
    st.markdown('Aside from the obvious benefit for people wanting to convert text from lower, upper, and sentence cases, this application could be utilized as an "input cleaner" for AI prior to the interpretation of input text, leading to more reliable language processing and named entity recognition.')
...
if input_sentence == '':
    ...
    i_capped = re.sub("( )(i)( |\?)", celtic_basic_capitalizer, Sents_Capped, flags=re.IGNORECASE)
    with open('first_names_resub') as firstnames:
        first_names = firstnames.read()
        first_names_capped = re.sub(f"( )({first_names})( |\'|\'s|\.|\?|\!)", mid_capitalizer, i_capped, flags=re.IGNORECASE)
    ...
    Basics_Capped = Capitalize_Word2(NLP_Dict_1, Capitalize_Word1(NLP_Dict_1, first_names_capped))
